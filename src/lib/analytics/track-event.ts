...
function sanitizeMetadata(metadata: Record<string, unknown> = {}) {
  return Object.entries(metadata).reduce<Record<string, string | number | boolean>>((accumulator, [rawKey, value]) => {
    ...
    const isExplicitlySafeWhatsAppKey = ["message_type", "window_open", "retryable", "attempts", "template_status", "template_category", "language", "sync_count_range", "variable_count", "media_type", "mime_group", "file_size_range", "connection_source", "has_phone_number", "has_waba"].includes(key);
