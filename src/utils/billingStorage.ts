...
export function createDefaultSubscription(planId: CheckoutPlanId = "starter") {
  ...
  addBillingEvent({
    ...
    description: `Simulação local iniciada no ${subscription.planName}. A cobrança real é gerenciada pela Kiwify.`,
    type: "created"
  });
  return subscription;
}
...
export function updateSubscriptionPlan(planId: CheckoutPlanId) {
  ...
  addBillingEvent({
    ...
    description: `A assinatura local foi atualizada para ${plan.name}. Confirme a cobrança real no checkout da Kiwify.`,
    type: "plan_changed"
  });
...
export function cancelLocalSubscription() {
  ...
  addBillingEvent({
    ...
    description: "O status foi alterado apenas neste navegador. Para cancelar cobranças reais, use a área da Kiwify.",
    type: "canceled"
  });
